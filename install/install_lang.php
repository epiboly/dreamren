...
$i_message['install_lock'] = '您已安装过 ，如果需要重新安装，请先删除install目录下的install.lock文件';
$i_message['install_title'] = ' 安装向导';
$i_message['install_wizard'] = '安装向导';
$i_message['install_warning'] = '<strong>注意 </strong>这个安装程序仅仅用在你首次安装.如果你已经在使用 或者要更新到一个新版本，请不要运行这个安装程序。';
$i_message['install_intro'] = '<h4>安装须知</h4><p>一、运行环境需求：PHP(5.2.0+)+MYSQL(4.1+)</p><p>二、安装步骤：<br /><br />1、使用ftp工具以二进制模式，将该软件包里的 wekit 目录及其文件上传到您的空间，假设上传后目录仍旧为 wekit。<br /><br />2、如果您使用的是Linux 或 Freebsd 服务器，先确认以下目录或文件属性为 (777) 可写模式。<br /><br />目录: data<br />目录: _runtime<br />目录: install<br />文件: config.inc.php<br />文件: define.inc.php<br />3、运行 http://yourwebsite/wekit/install/install.php 安装程序，填入安装相关信息与资料，完成安装！<br />4、运行 http://yourwebsite/wekit/cleancache.php 清除系统缓存文件！<br />5、运行 http://yourwebsite/wekit/index.php 开始体验！</p>';
$i_message['install_start'] = '开始安装';
$i_message['install_license_title'] = '安装许可协议';
$i_message['install_license'] = '版权所有 (C) 2008-'.date('Y').'，wekit.org 保留所有权利。
';
$i_message['install_agree'] = '我已看过并同意安装许可协议';
$i_message['install_disagree'] = '不同意';
$i_message['install_disagree_license'] = '您必须在同意授权协议的全部条件后，方可继续安装';
...
$i_message['mysql_unsupport'] = '您的服务器不支持MYSQL数据库，无法安装。';
...
$i_message['install_mysql_prefix_intro'] = '同一数据库安装多个时可改变默认值';
...
$i_message['wekit_rebuild'] = '数据库中已经安装过 ，继续安装会清空原有数据！';
...
$i_message['install_success_intro'] = '<p>安装程序执行完毕，请尽快删除整个 install 目录，以免被他人恶意利用。如要重新安装，请删除本目录的 install.lock 文件！</p><p><a href="../index.php">请点击这里开始体验吧！</a></p>';
$i_message['install_dbFile_error'] = '数据库文件无法读取，请检查/install/wekit.sql是否存在。';