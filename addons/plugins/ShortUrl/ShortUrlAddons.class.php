...
class ShortUrlAddons extends SimpleAddons
{
	...
	public function getShortUrl($param)
	{
		...
		//使用bit.ly短网址
		if($data['shorturlapi']=='bitly'){
			include_once(dirname(__FILE__).'/hooks/BitlyShortUrlHooks.class.php');
			$shorten = new BitlyShortUrlHooks;
			(false != $short = $shorten->getShortUrl($param['url'],$data['bitlylogin'],$data['bitlykey'])) && $param['url'] = $short;
			return ;
		}
